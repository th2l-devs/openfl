...
class CairoTextField
{
	public static function render(textField:TextField, renderer:CairoRenderer, transform:Matrix):Void
	{
		...
		// textBounds clips the surface down to a rectangle containing only the text, which is
		// cheaper to allocate and fill. It is derived from advance widths, though, so any font
		// whose ink reaches past them - an oblique display face, a swash, a heavy negative side
		// bearing - gets its edge glyphs cut off, and nothing in the layout data says whether
		// that is happening. Drawing into the full field rectangle is the only safe default.
